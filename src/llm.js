...
function normalizeStoryToolCalls(calls) {
  ...
  return rawCalls
    .map((call, index) => {
      ...
      if (!isRecognizedStoryTool(tool)) {
        const rollRandomHint = /roll_random/i.test(tool) ? 'roll_random 是随机数接口工具，只能通过 tool_calls 通道调用，不能放进最终 JSON 的 storyProgressToolCalls。' : '';
        throw new RetryableLlmFormatError(`storyProgressToolCalls[${index}].tool 未知：${tool}。只能使用 updateCharacter 或 recordNoStateChange。${rollRandomHint}`);
      }
  ...
  try {
    ...
    for (let attempt = 1; attempt <= MAX_TOOL_JSON_RETRY_REQUESTS; attempt += 1) {
      ...
      try {
        ...
        messages.push({
          role: 'user',
          content: `上一次回复中的 storyProgressToolCalls / toolCalls 无效：${error.message}。请重新输出完整最终 JSON：storyProgressToolCalls 必须是非空数组，且每一项 tool 只能是 updateCharacter 或 recordNoStateChange；roll_random 只能通过接口工具调用通道使用，绝对不能写进最终 JSON 的 storyProgressToolCalls。有状态变化时使用 {"tool":"updateCharacter","args":{...}}，没有任何状态变化时使用 {"tool":"recordNoStateChange","args":{"reason":"无状态变化但剧情仍推进的原因"}}；args 必须是 JSON 对象；不要把该字段写成字符串、Markdown 或自然语言。`,
        });
      }
    }
