...
- LLM Bot 队友：房主可在等待/已结束状态添加或移除 LLM Bot；Bot 会生成角色、在回合中自动提交队友行动，也能按信息权限发送/回应聊天并可选择沉默，避免 Bot 之间无限刷屏。
...
3. 进入第 1 回合：所有玩家提交行动；已提交者可在 LLM 结算开始前、且仍有其他可行动玩家未提交时撤回并重新提交。
4. 满足以下任一条件时结算：
